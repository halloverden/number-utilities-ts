...
test( 'getLuhnControlDigit() should create correct control digit', () => {
  ...
  values.forEach( ( v ) => {
    const output = getLuhnControlDigit( v.input );
...
test( 'getLuhnControlDigits() should return an array of correct control digits', () => {
  ...
  values.forEach( ( v ) => {
    const output = getLuhnControlDigits( v.input, v.numberOfDigits );
    expect( output ).toEqual( v.expectedOutput );
  } );
...
test( 'checkLuhnControlDigit() should validate the last control digit', () => {
  ...
  values.forEach( ( v ) => {
    const output = checkLuhnControlDigit( v.input );
    expect( output ).toEqual( v.expectedOutput );
  } );
...
test( 'checkLuhnControlDigits() should validate the given number of control digits', () => {
  ...
  values.forEach( ( v ) => {
    const output = checkLuhnControlDigits( v.input, v.numberOfDigits );
    expect( output ).toEqual( v.expectedOutput );
  } );
